...
const ManagerTenantManagement: React.FC = () => {
  const { user } = useAuth();
  ...
  const loadTenants = async () => {
    try {
      setIsLoading(true);
      // Get tenants by property owner using the new API endpoint
      const data = await tenantsAPI.getByPropertyOwner(user!.id);
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-8 bg-gradient-to-br from-emerald-50 to-white rounded-2xl shadow-lg font-montserrat">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">Manage tenants for your properties</p>
        </div>
        <button
...
export default ManagerTenantManagement;